# Проект SortFile отвечает за сортировку файлов с большими данными. 
# Он использует внешнюю сортировку для обработки файлов, которые не могут поместиться в оперативной памяти целиком.
...
# Примечание :
   - Метод `mergeSortedFiles` использует приоритетную очередь (PriorityQueue), чтобы гарантировать, что строки из разных файлов будут записаны 
   - в выходной файл в правильном порядке. Приоритетная очередь автоматически сортирует элементы по значению `currentLine`.
